  ...
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto space-y-6">
        ...
        >
          {profile?.avatar_url ? (
            <img 
              ...
              alt={displayName}
              className="h-20 w-20 rounded-full object-cover border-2 border-gray-100 group-hover:border-[#8b5cf6] transition-colors"
            />
          ) : (
            <div className="h-20 w-20 rounded-full bg-[#8b5cf6] flex items-center justify-center group-hover:bg-[#8b5cf6]/80 transition-colors">
              <User className="h-10 w-10 text-[#8b5cf6]-foreground" />
            </div>
          )}
          <div className="absolute bottom-0 right-0 h-7 w-7 rounded-full bg-card border-2 border-gray-100 flex items-center justify-center shadow-sm group-hover:border-[#8b5cf6] transition-colors">
            <Camera className="h-3.5 w-3.5 text-gray-500 group-hover:text-[#8b5cf6]" />
          </div>
          <input
            ref={avatarInputRef}
            type="file"
        ...
        <div className="flex-1 text-center sm:text-left">
          {isLoading ? (
            ...
            <>
              ...
              {profile?.bio && (
                <p className="text-xs text-gray-500 mt-1 italic">{profile.bio}</p>
              )}
            </>
          )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { icon: BookOpen,  label: "Lessons Done",   value: stats?.lessonsCompleted ?? 0,  color: "text-[#8b5cf6]" },
      ...
      {(stats?.badges?.length ?? 0) > 0 && (
        <div className="bg-white border border-gray-100 shadow-sm rounded-2xl p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Link to="/achievements" className="text-xs text-[#8b5cf6] hover:underline flex items-center gap-1">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Link
          to="/progress"
          className="flex items-center gap-3 bg-white border border-gray-100 shadow-sm rounded-2xl p-4 hover:border-[#8b5cf6]/40 transition-colors group"
        >
          ...
          <ArrowRight className="h-4 w-4 text-gray-500 ml-auto group-hover:text-[#8b5cf6] transition-colors" />
        </Link>
        ...
        >
          ...
          <div>
            ...
            <div className="text-xs text-gray-500">Badges & rewards</div>
          </div>
          <ArrowRight className="h-4 w-4 text-gray-500 ml-auto group-hover:text-[#f4a261] transition-colors" />
        </Link>
      </div>
    </div>
  );
